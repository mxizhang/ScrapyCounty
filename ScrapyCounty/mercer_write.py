...
def mercer_write(SS_ADDRESS, key):
	...
	print "------------------------------------------"
	print "Mercer County has " + str(row_count) + " items!"
	print "------------------------------------------"
	...
	for line in data[1:]:
		...
		if line[0] is not "":
			try:
				...
				worksheet.update_cell(cell.row, 1, line[0] + "->" + line[3]) #date
				worksheet.update_cell(start, 9, line[5]) #upset
				...
				worksheet.update_cell(start, 1, line[0] + "->" + line[3]) #date
				worksheet.update_cell(start, 2, line[1]) #shf no
				worksheet.update_cell(start, 3, line[2]) #case
				worksheet.update_cell(start, 4, "PLF: " + line[4] + "\n" + "DEF: " + line[6])
				worksheet.update_cell(start, 5, line[7]) #att
				worksheet.update_cell(start, 6, line[8]) #add
				worksheet.update_cell(start, 9, line[5]) #upset
				address = line[8]
